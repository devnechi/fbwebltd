$(document).ready(function() {
 ...
 console.log('on hoover')
	...
	//Optimize the interaction scheme, use the before pseudo-class, and tile horizontally, 
	// so that the sub-menu does not need to be triggered only when the menu item is triggered
	// .menu_list .menu_item .txt:before

	//Desktop connection module
	...
	/* Connection responsibility and trust banner */
	...
	/* Connecting talents and development banner */
	...
	function pageTab(){
		var deviceWidth = $(window).width() // Device screen width
	...
	var brandTabOffTop = [];
	$(window).resize( function() { 
		// When the page width on the PC side is reduced to the size of the mobile side
		$('.mod-tab .clearfix ul').css({'left':'16px','right': 'auto'});
		pageTab();
		brandTabOffTop = [];
	...
	setTimeout(function(){
		...
		// Set the initial height of the hypotenuse triangle on the epidemic page
		var sanWidth = $('.artice-box .san').width() - 1;
		$('.artice-box .san').css('right',-sanWidth+'px');
